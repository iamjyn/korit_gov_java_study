...
public class Tiger extends Animal {
    ...
    // 그러므로 자식클래스에서 추가한 필드 속성도 getter/setter를 만들어줘야 한다.
    private boolean isStriped;

    public Tiger(String animalName, int animalAge, boolean isStriped) {
        super(animalName, animalAge);
        this.isStriped = isStriped;
        // 고유 필드속성을 대입하기 전에 부모의 속성을 먼저 생성자 호출해야 한다.
    }
    ...
    // 부모 클래스의 메소드를 자식 클래스에서 동일한 이름, 매개변수, 리턴타입으로 재정의하는 것
    // 왜? => 자식 클래스에 맞게 다르게 동작시켜야 할 때
    // 부모 클래스의 접근제어범위가 더 좁아야 한다.

    /* 어노테이션(시그니처) : 코드에 붙이는 꼬리표
    * 이 친구는 이런 용도로 사용되니까 좀 특별하게 유심히 잘 봐줘
    * 코드를 읽고 컴퓨터가 이해하도록 어노테이션을 달아주면
    * 컴퓨터가 어노테이션을 보고 인식함
    * */
    @Override
    public void move() {
        super.move();
        // 부모 클래스인 Animal 클래스의 move() 메소드를 호출한 것
